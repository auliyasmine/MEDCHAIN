...
const Wallet = (() => {
  ...
  const Contract = {
    ...
    async call(contractAddress, abi, method, ...args) {
      ...
      // PENTING: pakai _signer (kalau wallet sudah terhubung) alih-alih _provider
      // polos. Beberapa fungsi view di kontrak (mis. getRecord, getPatientInfo)
      // punya access control berbasis msg.sender (require(msg.sender == patient
      // || hasAccess[...])). Kalau instance kontrak dibuat dari _provider saja,
      // eth_call dikirim tanpa "from" yang benar sehingga msg.sender di kontrak
      // jadi address(0) dan getter itu selalu revert untuk pemilik aslinya
      // sekalipun. Pakai _signer memastikan "from" terisi alamat wallet yang
      // benar-benar terhubung.
      const connector = _signer || _provider;
      const contract = new ethers.Contract(contractAddress, abi, connector);
