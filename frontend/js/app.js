...
const PatientApp = {
  async init() {
    // PENTING: kalau sudah live (wallet+contract terkonfigurasi), JANGAN percaya
    // cache localStorage lama (dari Store/hydrateMockData) sebagai sumber
    // kebenaran untuk records. Cache itu bisa berisi data palsu peninggalan versi
    // lama aplikasi (sebelum upload betul-betul terhubung ke backend), dan kalau
    // dibiarkan akan terus tampil sebagai "Menunggu Verifikasi" selamanya karena
    // tidak berkorelasi dengan baris apapun di database. Kosongkan dulu di sini,
    // baru diisi ulang dari on-chain (loadRealRecords) + backend (loadPendingUploads).
    if (Bridge.isLive()) {
      MockData.records = [];
    }
  ...
  async loadRealRecords() {
    try {
      ...
      UI.Toast.error('Gagal Membaca Rekam Medis On-chain', err.message);
    }
  },

  /** Ambil status dokumen yang PERNAH diunggah pasien (pending & verified) langsung
   *  dari backend (tabel pending_uploads), lalu gabungkan ke MockData.records.
   *  Ini yang membuat status "Menunggu Verifikasi" berubah jadi "Terverifikasi"
   *  begitu dokter konfirmasi — sebelumnya status ini hanya disimpan di
   *  localStorage sisi pasien sehingga tidak pernah ikut ter-update. */
  async loadPendingUploads() {
    try {
      const account = Wallet.getAccount() || localStorage.getItem('mc_account');
      if (!Bridge.isLive() || !account) return; // mode demo tetap pakai data lokal

      const res = await Bridge.api(`/records/mine/${account}`);
      const uploads = res.data || [];

      // Record yang statusnya 'verified' di tabel pending_uploads sudah pasti
      // juga muncul dari on-chain getRecord() lewat loadRealRecords() di atas,
      // jadi di sini kita hanya perlu menambahkan yang MASIH 'pending' supaya
      // tidak dobel — dan supaya begitu dokter verifikasi, baris ini otomatis
      // hilang dari daftar "pending" dan digantikan versi terverifikasi on-chain.
      const stillPending = uploads
        .filter(u => u.status === 'pending')
        .map(u => ({
          id: u.id,
          date: u.createdAt ? new Date(Number(u.createdAt) * 1000).toLocaleDateString('id-ID') : '—',
          doctor: u.doctorAddress,
          hospital: '(menunggu verifikasi)',
          diagnosis: u.note ? `Menunggu peninjauan dokter — ${u.note}` : 'Menunggu peninjauan dokter',
          ipfsHash: u.ipfsCID,
          txHash: null,
          patientAddress: u.patientAddress,
          status: 'pending',
        }));

      // Ganti seed/mock lama dan gabungkan dengan record on-chain yang sudah ada.
      MockData.records = [...stillPending, ...MockData.records];
    } catch (err) {
      console.warn('Gagal memuat status upload dari backend:', err.message);
    }
  },
  ...
  renderRecords() {
    ...
    if (!MockData.records.length) {
      tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:32px;color:var(--color-navy-400)">
        <i class="fas fa-folder-open"></i> Belum ada rekam medis. Kalau menurut Anda seharusnya sudah ada,
        coba klik "Refresh dari Blockchain" atau cek console browser untuk detail error.
      </td></tr>`;
      return;
    }
  ...
  bindEvents() {
    // Handler form upload dokumen pasien → kirim ke backend → muncul di dokter.
    // CATATAN: ini SATU-SATUNYA handler submit untuk #form-upload. Sebelumnya ada
    // handler kedua (inline di patient.html) yang membuat record palsu langsung
    // di localStorage tanpa terhubung ke backend, sehingga statusnya tidak pernah
    // ikut ter-update saat dokter memverifikasi. Handler itu sudah dihapus.
    document.getElementById('form-upload')?.addEventListener('submit', async (e) => {
      e.preventDefault();
      const btn = e.target.querySelector('button[type=submit]');
      const restore = UI.setButtonLoading(btn, 'Mengunggah…');

      const fileInput   = document.getElementById('upload-file') || document.querySelector('#form-upload input[type=file]');
      const doctorInput = document.getElementById('upload-doctor');
      ...
      const doctorAddr  = doctorInput?.value.trim();
      const note        = noteInput?.value.trim();
      const docType     = typeInput?.value.trim();
      const docDate     = dateInput?.value;
      const account     = Wallet.getAccount() || localStorage.getItem('mc_account');

      // Catatan yang dikirim ke dokter HARUS murni dari kolom "Catatan untuk
      // Dokter" saja — jangan dicampur dengan jenis dokumen/tanggal, supaya
      // "Catatan Pasien" yang tampil di portal dokter sesuai persis dengan
      // yang benar-benar diketik pasien.

      // Step bar (opsional, hanya ada di halaman upload)
      const steps = ['step-1', 'step-2', 'step-3'];
      const setStepActive = (i) => {
        document.getElementById(steps[i])?.classList.add('active');
        if (i > 0) document.getElementById(`line-${i}`)?.classList.add('done');
      };
      const setStepDone = (i) => {
      ...
      const resetSteps = () => {
        ...
        ['line-1', 'line-2'].forEach(id => document.getElementById(id)?.classList.remove('done'));
      };

      try {
        if (!file)       throw new Error('Pilih file dokumen terlebih dahulu');
        if (!doctorAddr) throw new Error('Masukkan alamat wallet dokter');
        if (!account)    throw new Error('Hubungkan wallet pasien terlebih dahulu');

        if (Bridge.isLive()) {
          setStepActive(0);
          const t1 = UI.Toast.loading('Mengenkripsi & mengunggah ke IPFS…', 'Melalui backend MedChain');

          const form = new FormData();
          form.append('document',       file);
          form.append('doctorAddress',  doctorAddr);
          form.append('patientAddress', account);
          if (note) form.append('note', note);

          await Bridge.api('/records/upload', { method: 'POST', body: form });
          UI.Toast.dismiss(t1);
          setStepDone(0); setStepActive(1);
          setStepDone(1); setStepActive(2);
          setStepDone(2);

          UI.Toast.success('Dokumen Dikirim!', 'Dokter akan melihat permintaan verifikasi Anda.');

          // Muat ulang status pending langsung dari backend supaya baris "Menunggu
          // Verifikasi" yang baru dikirim langsung tampil di tabel rekam medis.
          await PatientApp.loadPendingUploads();
          PatientApp.renderRecords();
        } else {
          // Mode demo
          setStepActive(0);
          ...
          await new Promise(r => setTimeout(r, 600));
          setStepDone(2);
          UI.Toast.success('Dokumen Dikirim! (Demo)', 'Di mode live, dokumen akan muncul di portal dokter.');
        }

        e.target.reset();
        document.getElementById('file-name')?.style && (document.getElementById('file-name').style.display = 'none');
        resetSteps();
      } catch (err) {
        UI.Toast.error('Gagal Upload', err.message);
      } finally { restore(); }
    });
...
const DoctorApp = {
  ...
  // Cache pendingRequests dari backend agar openVerifyModal bisa akses data
  _pendingCache: [],
  ...
  async renderPendingRequests() {
    ...
    const currentDoctor = (Wallet.getAccount() || localStorage.getItem('mc_account') || '').toLowerCase();
    ...
    // Ambil dari backend jika live, fallback ke MockData jika demo
    let pending = [];
    if (Bridge.isLive()) {
      try {
        const res = await Bridge.api(`/records/pending?doctorAddress=${currentDoctor}`);
        pending = (res.data || []).map(r => ({
          id:            String(r.id),
        ...
        this._pendingCache = pending;
      } catch (err) {
        console.warn('Gagal ambil pending dari backend:', err.message);
      }
    } else {
      pending = MockData.records.filter(r =>
        r.status === 'pending' &&
    ...
    if (!pending.length) {
      tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;padding:32px;color:var(--color-navy-400)">
    ...
    tbody.innerHTML = pending.map(r => `
      <tr>
        ...
        <td style="max-width:260px;font-size:0.8125rem;color:var(--color-navy-400)">${UI.escapeHtml(r.note || '—')}</td>
  ...
  openVerifyModal(id) {
    // Coba ambil dari cache backend dulu, fallback ke MockData
    const r = this._pendingCache.find(x => x.id === id) || MockData.records.find(x => x.id === id);
    if (!r) return;
  ...
  async verifyRecord(id, diagnosis, hospital) {
    // Ambil dari cache backend atau MockData
    const rBackend = this._pendingCache.find(x => x.id === id);
    const rMock    = MockData.records.find(x => x.id === id);
    const r = rBackend || rMock;
    if (!r) return;

    if (Bridge.isLive() && rBackend) {
      // Mode live: dokter menandatangani addMedicalRecord() lewat MetaMask
      const t1 = UI.Toast.loading('Menandatangani transaksi…', 'MetaMask akan meminta konfirmasi');
      let txHash;
      try {
        txHash = await Bridge.sendTx(
          'addMedicalRecord',
          [rBackend.patientAddress, rBackend.ipfsHash, rBackend.documentHash, diagnosis],
          'addMedicalRecord(address,string,bytes32,string)'
        );
      } catch (err) {
        UI.Toast.dismiss(t1);
        UI.Toast.error('Gagal Verifikasi', err.message);
        return;
      }
      UI.Toast.dismiss(t1);

      // Tandai sebagai verified di backend. PENTING: transaksi on-chain sudah
      // berhasil di titik ini, tapi kalau langkah sinkronisasi status ke
      // database ini gagal, JANGAN diam-diam diabaikan — kalau tidak, status
      // di dashboard pasien akan nyangkut "Menunggu Verifikasi" selamanya
      // walau transaksinya sendiri sudah sukses di blockchain.
      try {
        await Bridge.api(`/records/pending/${id}/verify`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ txHash }),
        });
        UI.Toast.success('Rekam Medis Terverifikasi!', `Tx: ${UI.shortHash(txHash)}`);
      } catch (err) {
        UI.Toast.error(
          'Transaksi Berhasil, Tapi Sinkron Gagal',
          `Tx ${UI.shortHash(txHash)} sukses on-chain, tapi status di server gagal diperbarui: ${err.message}. Coba klik Verifikasi lagi.`
        );
        return; // jangan tutup modal / refresh list seolah-olah sudah beres
      }
    } else {
      // Mode demo
      ...
      if (rMock) {
        rMock.status    = 'verified';
        rMock.diagnosis = diagnosis;
        rMock.doctor    = localStorage.getItem('mc_account') ? `Anda (${Wallet.shortAddress(localStorage.getItem('mc_account'))})` : 'Anda';
        rMock.txHash    = txHash;
        Store.save();
      }
      UI.Toast.success('Rekam Medis Terverifikasi!', `Tx: ${UI.shortHash(txHash)}`);
  ...
  bindEvents() {
    ...
    document.getElementById('form-add-record')?.addEventListener('submit', async (e) => {
      ...
      const file = fileInput?.files?.[0];
      const doctorAddr = Wallet.getAccount() || localStorage.getItem('mc_account');

      let t1 = null; // loading toast — dipegang di scope luar supaya SELALU bisa di-dismiss di catch/finally
      try {
        if (!patientAddr) throw new Error('Masukkan alamat wallet pasien');
        if (!UI.isValidAddress(patientAddr)) throw new Error('Alamat wallet pasien tidak valid');
        if (!diagnosis) throw new Error('Isi diagnosis terlebih dahulu');

        let ipfsCID, documentHash;

        if (Bridge.isLive()) {
          if (!file) throw new Error('Pilih dokumen rekam medis terlebih dahulu');
          if (!doctorAddr) throw new Error('Hubungkan wallet dokter terlebih dahulu');

          // Step 1: Upload dokumen ke backend -> backend yang teruskan ke IPFS (Pinata)
          t1 = UI.Toast.loading('Mengunggah ke IPFS…', 'Lewat backend MedChain');
          const form = new FormData();
          form.append('document', file);
          form.append('doctorAddress', doctorAddr);
          form.append('patientAddress', patientAddr);
          ...
          // Karena dokter yang langsung menambahkan & menandatangani rekam ini
          // sendiri (bukan verifikasi upload dari pasien), langsung tandai baris
          // pending_uploads terkait sebagai 'verified' juga — supaya TIDAK
          // nyangkut selamanya di kotak masuk "Permintaan Verifikasi" milik
          // dokter itu sendiri.
          if (uploadId != null) {
            await Bridge.api(`/records/pending/${uploadId}/verify`, {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ txHash }),
            }).catch(err => console.warn('Gagal sinkron status verified:', err.message));
          }
          ...
          t1 = UI.Toast.loading('Mengenkripsi dokumen…', 'Mempersiapkan upload ke IPFS');
        ...
        if (t1) UI.Toast.dismiss(t1); // pastikan toast loading TIDAK nyangkut kalau ada error
